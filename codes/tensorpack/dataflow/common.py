...
__all__ = ['TestDataSpeed', 'PrintData', 'BatchData', 'BatchDataByShape', 'FixedSizeData', 'MapData', 'CastData',
...
class CastData(ProxyDataFlow):
    ...
    def __init__(self, ds, func):
        ...
        Note:
            ...
            unless you're certain it's safe.
        """
        super(CastData, self).__init__(ds)
        self.func = func

    def get_data(self):
        sz = self.ds.size()
        tmp = 0
        ds_data = list(self.ds.get_data())
        for idx, dp in enumerate(ds_data):
            while True:
                tmp = np.random.randint(sz)
                if tmp != idx:
                    break
            ret = self.func(dp, ds_data[tmp])
            if ret is not None:
                yield ret
