...
def weighted_loss(logits, labels, num_classes, head=None, name='weighted_loss'):
    ...
    loss = tf.add_n(tf.get_collection('losses'), name=name)
    return loss 

def cal_weighted_loss(logits, labels, name):
    '''
    ...
    return weighted_loss(logits, labels, 6, loss_weight, name)
    '''
    loss_weight = np.asarray([1.0, 33.331920])
    labels = tf.cast(labels, tf.int32)
    return weighted_loss(logits, labels, num_classes=2, head=loss_weight, name=name)
