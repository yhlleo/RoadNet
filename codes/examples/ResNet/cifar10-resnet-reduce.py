...
class Model(ModelDesc):
    ...
    def _build_graph(self, inputs):
        ...
        def random_blur(x):
          def gauss_kernel(kernlen=3, sigma=1.0, channels=1):
            ...
            return out_filter
          def blur(x, name='gauss_wight'):
            c = x.get_shape().as_list()[3]
            conv = lambda i, k: tf.nn.depthwise_conv2d(i, k, [1, 1, 1, 1], padding="SAME")

            kernlen = random.randint(1, 2) * 2 + 1
            nsig = random.uniform(0.0, 1.0)
            with tf.variable_scope(name) as scope:
              kernel = gauss_kernel(kernlen=kernlen, sigma=nsig, channels=c)
              val = tf.Variable(tf.convert_to_tensor(kernel), name=name)
              return conv(x, val)
          return blur(x)


        '''
        def jpeg_noise(x, level_max=5):
          assert level_max < 100
          #for i in range(BATCH_SIZE):
          def body(x):
            if isTrue():
              x = tf.image.convert_image_dtype(x, dtype=tf.uint8, saturate=True)
              jpeg_quality = random.randint(level_max, 100-level_max)
              jpeg_code = tf.image.encode_jpeg(x, quality=jpeg_quality)
              x = tf.image.convert_image_dtype( \
                    tf.image.decode_jpeg(jpeg_code, channels=3), \
                    dtype=tf.float32)
          cond = 
        ...
        def gaussion_noise(x):
          mask = tf.random_normal([BATCH_SIZE, 32, 32, 3], 
                                  mean=0.0, stddev=0.5, name='mask') 
          mask = tf.clip_by_value(mask, -1.0, 1.0) * 255 
          beta = tf.random_uniform([BATCH_SIZE, 1, 1, 1], 
                                   minval=0., maxval=0.3, name='beta')
          x = x * (1-beta) + mask * beta
          return x

        def noisy(x):
            alpha = tf.random_uniform([BATCH_SIZE, 1, 1, 1], 
                                     minval=-0.1, maxval=0.1, name='alpha') + 1.0
            #x = random_blur(x*alpha)
            x = gaussion_noise(x*alpha)
