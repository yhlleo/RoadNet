...
class Model(ModelDesc):
    ...
    def _build_graph(self, inputs):
        ...
        def cross_entry_loss(name, pred, gt):
             return [tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(logits=pred, labels=gt), name=name)]
        '''
        ...
        segment_costs = cross_entry_loss('segment', segment_map, segment)

        merge_data = tf.concat([image, segment_map], 3)

        boundary_map = unetc('boundary', merge_data)
        boundary_costs = cross_entry_loss('boundary', boundary_map, boundary)

        skeleton_map = unetc('skeleton', merge_data)
        skeleton_costs = cross_entry_loss('skeleton', skeleton_map, skeleton)
