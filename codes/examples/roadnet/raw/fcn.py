    ...
    def _build_graph(self, inputs):
        ...
        def fcn8s(name, l):
            with argscope(Conv2D, kernel_shape=3, nl=tf.nn.relu), \
                argscope(Deconv2D, kernel_shape=4, stride=2):
                with tf.variable_scope(name):
                    ...
                    l = Deconv2D('deconv3', l, 2, kernel_shape=16, stride=8)
                    return l
        ...
        def fcnc(name, l):
            with argscope(Conv2D, kernel_shape=3, nl=tf.nn.relu), \
                ...
                with tf.variable_scope(name):
                    ...
                    l = Deconv2D('deconv3', l, 2, kernel_shape=8, stride=4)
                    return l
        ...
        boundary_map = fcnc('boundary', merge_data)
        boundary_costs = loss('boundary', boundary_map, boundary)

        skeleton_map = fcnc('skeleton', merge_data)
