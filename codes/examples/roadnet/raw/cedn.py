...
# param=124867971 (476.33 MB assuming all float32)
#
...
class Model(ModelDesc):
    ...
    def _build_graph(self, inputs):
        ...
        def cedn(name, l):
            with argscope(Conv2D, kernel_shape=3, nl=tf.nn.relu), \
                ...
                with tf.variable_scope(name):
                    ...
                    l = Conv2D('conv6_1', l, 4096, kernel_shape=7, padding='SAME')
                    l = Dropout(l, 0.5)
                    ...
                    l = Conv2D('pred', l, 2, kernel_shape=5, nl=tf.identity)
                return l
        ...
        segment_map = cedn('segment', image)
...
def run(model_path, image_path, output):
    ...
    print 'average infer time (ms): %.2f'%(sum(time_consum)/len(time_consum)) 
