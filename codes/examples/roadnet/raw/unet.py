...
#Total #param=55297603 (210.94 MB assuming all float32)
...
class Model(ModelDesc):
    ...
    def _build_graph(self, inputs):
        ...
        def branch(name, x, y, num):
            xy = tf.concat([x, y], 3)
            with tf.variable_scope(name) as scope:
                l = Conv2D('convfc', xy, num, kernel_shape=3)
                return l
                
        def unet(name, l):
            with argscope(Conv2D, kernel_shape=3, nl=tf.nn.relu), \
                argscope(Deconv2D, kernel_shape=2, stride=2):
                with tf.variable_scope(name):
                    ...
                    c3 = Conv2D('conv3_2', l, 256)
                    l = MaxPooling('pool3', c3, 2)

                    l = Conv2D('conv4_1', l, 512)
                    c4 = Conv2D('conv4_2', l, 512)
                    l = MaxPooling('pool4', c4, 2)

                    l = Conv2D('conv5_1', l, 1024)
                    l = Conv2D('conv5_2', l, 1024)
                    
                    l = Deconv2D('deconv5', l, 512)
                    l = branch('concat4', c4, l, 512)                   
                    l = Conv2D('deconv4_2', l, 512)
                    l = Conv2D('deconv4_1', l, 512)
                               
                    l = Deconv2D('deconv4', l, 256)
                    l = branch('concat3', c3, l, 256) 
                    l = Conv2D('deconv3_2', l, 256)
                    l = Conv2D('deconv3_1', l, 256)
                    
                    l = Deconv2D('deconv3', l, 128)
                    l = branch('concat2', c2, l, 128) 
                    l = Conv2D('deconv2_2', l, 128)
                    l = Conv2D('deconv2_1', l, 128)
                    
                    l = Deconv2D('deconv2', l, 64)
                    l = branch('concat1', c1, l, 64) 
                    l = Conv2D('deconv1_3', l, 64)
                    l = Conv2D('deconv1_2', l, 64)
                    l = Conv2D('deconv1_1', l, 2, kernel_shape=1, nl=tf.identity)
                    return l              
        ...
        def unetc(name, l):
            with argscope(Conv2D, kernel_shape=3, nl=tf.nn.relu), \
                ...
                with tf.variable_scope(name):
                    ...
                    c3 = Conv2D('conv3_2', l, 128)
                    l = MaxPooling('pool3', c3, 2)

                    l = Conv2D('conv4_1', l, 256)
                    l = Conv2D('conv4_2', l, 256)
                               
                    l = Deconv2D('deconv4', l, 128)
                    l = branch('concat3', c3, l, 128) 
                    l = Conv2D('deconv3_2', l, 128)
                    l = Conv2D('deconv3_1', l, 128)
                    
                    l = Deconv2D('deconv3', l, 64)
                    l = branch('concat2', c2, l, 64) 
                    l = Conv2D('deconv2_2', l, 64)
                    l = Conv2D('deconv2_1', l, 64)
                    
                    l = Deconv2D('deconv2', l, 32)
                    l = branch('concat1', c1, l, 32) 
                    l = Conv2D('deconv1_3', l, 32)
                    l = Conv2D('deconv1_2', l, 32)
                    l = Conv2D('deconv1_1', l, 2, kernel_shape=1, nl=tf.identity)
                    return l 
                    

        def loss(name, logits, labels, num_classes=2, head=None):
            with tf.name_scope(name):
                ...
                cross_entropy_mean = tf.reduce_mean(cross_entropy, name='entropy_mean')
                tf.add_to_collection('losses', cross_entropy_mean)
                loss = tf.add_n(tf.get_collection('losses'), name='ec_loss')
            return [loss]
        
        segment_map = unet('segment', image)
        segment_costs = loss('segment', segment_map, segment)

        merge_data = tf.concat([image, segment_map], 3)

        boundary_map = unetc('boundary', merge_data)
        boundary_costs = loss('boundary', boundary_map, boundary)

        skeleton_map = unetc('skeleton', merge_data)
        skeleton_costs = loss('skeleton', skeleton_map, skeleton)
