...
class Model(GANModelDesc):
    ...
    def _build_graph(self, inputs):
        ...
        if self.isTrain:
            '''
            alpha = tf.random_uniform(shape=[BATCH, SHAPE_H, SHAPE_W, IN_CH],
                                      minval=0., maxval=1., name='alpha')
            beta = tf.random_uniform(shape=[BATCH, SHAPE_H, SHAPE_W, IN_CH],
                                 minval=0., maxval=1., name='beta')
            '''
            alpha = tf.random_normal(shape=[BATCH, SHAPE_H, SHAPE_W, IN_CH],
                                     mean=0., stddev=0.5, name='alpha')
            alpha = tf.clip_by_value(alpha, -1.0, 1.0) 
            beta = tf.random_normal(shape=[BATCH, SHAPE_H, SHAPE_W, IN_CH],
                                    mean=0., stddev=0.5, name='beta')
            beta = tf.clip_by_value(beta, -1.0, 1.0) 
                      
            gamma = tf.random_uniform(shape=[BATCH, 1, 1, 1],
                                      minval=0., maxval=1., name = 'gamma') * 255
            new_input = tf.multiply(1.0 - tf.image.grayscale_to_rgb(output), alpha) * gamma
            new_output = tf.multiply(tf.image.grayscale_to_rgb(output), beta) * gamma
            
            epsilon = tf.random_uniform(shape=[BATCH, 1, 1, 1],
                                        minval=0., maxval=1., name = 'epsilon') * 0.3            
            input = input * (1.0-epsilon) + new_input * (epsilon/2.0) + new_output * (epsilon/2.0)
            #input = input * 0.8 + new_input * 0.1 + new_output * 0.1
        ...
        if OUT_CH == 1:
            ...
            fake_output = tf.image.grayscale_to_rgb(fake_output) * 255.
